...
- **[第三束（双光子）探测与编码报错修复（2026-09-24）](docs/probe20260924/第三束探测.md)**：三脉冲输入、He+ 探测设计与收敛、快速离子因子化路径及其验证；超算 `UnicodeEncodeError: 'ascii'` 的原因、修复与已完成传播任务的恢复方法。
- [独立最小交接代码包（20260924 版，约 11.8 MB）](deliveries/README.md)：无需 Git 和原服务器路径，含中文菜单、编译测试、两脉冲与三脉冲小算例、谱图重画与 Slurm 入口；已在 ASCII locale 下独立解压实跑核验。
...
三脉冲（第三束探测）输入在 `configs/probe_20260924/`；提交方式与两脉冲相同，设 `HELIUM_PLAN=configs/probe_20260924/production/plan.json`。
