...
def extract(h,flux,times,avec,labels,energy,directions,dt,substeps=1):
    ...
    ion=Ionic(h,magnetic);chi=ion.final_states(labels)
    ...
    outstates=sorted(set(a for a,b in h.ch))
    tests={lm:np.sqrt(2/np.pi)*(-1j)**lm[0]*np.sqrt(w)[None,:]*r[None,:]*spherical_jn(lm[0],k[:,None]*r[None,:]) for lm in outstates}
    harmonics={lm:sph_harm_y(lm[0],lm[1],theta,phi) for lm in outstates}
    for i in range(len(times)-1,-1,-1):
        integral=np.zeros_like(b);v=chi.reshape(ion.na,h.n,len(labels))
        for c,(inner,outer) in enumerate(h.ch):
            if inner not in ion.index:continue
            proj=v[ion.index[inner]].conj().T@flux[i,:,:,c]
            integral+=np.einsum('ks,as,j->akj',tests[outer],proj,harmonics[outer])
