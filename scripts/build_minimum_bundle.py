...
PACKAGE='laqphysics-minimum-20260924'
PREFIXES=('python/','src/','tests/','configs/preparation_20260921/','docs/iteration20260921/',
          ...
          'configs/probe_20260924/','docs/probe20260924/','results/probe_20260924/')
SCRIPTS=('simulate_spectrum.py','extract_projected.py','refine_online_spectrum.py','refine_preparation_spectra.sh',
         ...
         'make_probe_campaign.py','probe_ion_design.py','apply_ionic_probe.py','compare_probe_spectra.py')

def main():
    p=argparse.ArgumentParser();p.add_argument('--output',type=Path,default=root/f'deliveries/{PACKAGE}.tar.gz')
    ...
    manifest={'format':1,'package':PACKAGE,'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip(),
              'working_tree_override_files':overrides,'files':files,'full_production_convergence_certified':False,
              'scope':'Source, local tests, six completed SI energy spectra, the three-pulse probe inputs and validation evidence, provenance and production inputs; build/runtime dependencies and historical restart arrays are supplied separately.'}
    ...
    with temporary.open('wb') as raw,gzip.GzipFile(filename='',mode='wb',fileobj=raw,mtime=0) as compressed,tarfile.open(mode='w',fileobj=compressed) as archive:
        for name,data in sorted(payload.items()):
            info=tarfile.TarInfo(PACKAGE+'/'+name);info.size=len(data);info.mode=modes[name];info.mtime=0
